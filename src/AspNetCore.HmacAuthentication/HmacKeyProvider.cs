...
/// </summary>
/// <remarks>
/// This provider looks up HMAC secrets from a configuration section, typically specified by <see cref="HmacAuthenticationOptions.SecretSectionName"/>.
...
public class HmacKeyProvider : IHmacKeyProvider
{
    ...
    private readonly IOptionsMonitor<HmacAuthenticationOptions> _options;
    ...
    /// </summary>
    /// <param name="clientId">The client identifier whose HMAC secret is to be retrieved. Must not be <c>null</c> or whitespace.</param>
    /// <returns>
    /// A <see cref="ValueTask{TResult}"/> containing the HMAC secret as a string, or <c>null</c> if not found.
    /// </returns>
    /// <remarks>
    /// The secret is retrieved from the configuration section specified by <see cref="HmacAuthenticationOptions.SecretSectionName"/>.
    /// If the secret is not found, a warning is logged and <c>null</c> is returned.
    /// </remarks>
    /// <exception cref="ArgumentException">
    /// Thrown if <paramref name="clientId"/> is <c>null</c>, empty, or consists only of white-space characters.
    /// </exception>
    public ValueTask<string?> GetSecretAsync(string clientId)
    {
        ...
        var secret = _configuration[$"{sectionName}:{clientId}"];

        if (string.IsNullOrEmpty(secret))
        {
            _logger.LogWarning("No HMAC secret found for client ID '{clientId}'.", clientId);
