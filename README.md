...
> **Production tip:** Storing secrets in `appsettings.json` is fine for local development and quick starts. For production, implement a [Custom Key Provider](#custom-key-provider) to load secrets from a secure store such as Azure Key Vault, a database, or another secrets manager.

### Client Setup (.NET HttpClient)

Use `AddHmacClient` to register an `HttpClient` whose outgoing requests are automatically signed with HMAC
authentication. It is the HMAC-enabled equivalent of `AddHttpClient`, and returns an `IHttpClientBuilder`
so you can chain additional configuration.
...
// Register a named HMAC client and set its base address
builder.Services.AddHmacClient("SecureApi", (sp, client) =>
{
...
> For typed clients, base address resolution, and runtime provisioning, see [Client Configuration](#client-configuration).
...
| **x-nonce**          | Unique per-request value (GUID/UUID). Included by default in signed headers and auto-generated by the .NET client. Not server-enforced, but strongly recommended — without it, identical requests within the same second are rejected when replay protection is enabled (the default) |
...
services.AddHmacClient(configureOptions: options =>
{
...
Use `AddHmacClient` to register an `HttpClient` whose outgoing requests are automatically signed with HMAC
authentication. It is the HMAC-enabled equivalent of `AddHttpClient` and returns an `IHttpClientBuilder` so you
can chain additional configuration. Options bind from the `HmacAuthentication` configuration section by default.

```csharp
// Default (unnamed) client — binds options from the "HmacAuthentication" section
services.AddHmacClient();

// Named client with base address and custom options
services.AddHmacClient("SecureApi",
...
`AddHmacClient` always assigns a base address to the underlying `HttpClient` so that relative request URIs
(such as `/api/secure`) are accepted. The base address is resolved in the following order:

1. **`HmacAuthenticationOptions.BaseAddress`** — bound from the `BaseAddress` value in the
   `HmacAuthentication` configuration section (or set via `configureOptions`). When present, it is used as the
   client's base address.
2. **`configureClient` override** — the `configureClient` delegate runs after the options-based base address is
   applied, so setting `client.BaseAddress` there takes precedence. This is useful when the address is not part
   of configuration.
3. **Deferred placeholder** — when no base address is available at startup, a placeholder
   (`http://hashgate.invalid/`) is assigned. `HmacAuthenticationHttpHandler` detects this placeholder at request
   time and rewrites the request URI using the current `BaseAddress` from options. This supports scenarios where
   the base address is provisioned *after* startup (see [Runtime Provisioning](#runtime-provisioning)).

Because the handler resolves `BaseAddress` from the options monitor on every request, later updates applied
through `OptionsUpdater<HmacAuthenticationOptions>` are observed automatically — even for clients created before
the base address was provisioned.

### Typed Clients

Register a typed `HttpClient` whose outgoing requests are automatically signed with HMAC. Each typed client
gets its own named `HmacAuthenticationOptions` (keyed by the client type), so multiple clients can use
different credentials and base addresses.

```csharp
// Binds options from the "HmacAuthentication" configuration section
services.AddHmacClient<MyApiClient>();

// Or bind from a custom section and configure the HttpClient
services.AddHmacClient<MyApiClient>("MyApi", (sp, http) =>
{
...
For clients that are provisioned *after* startup — for example when the base address, client id, and secret
are not known until they are loaded from an external source at runtime — register the client during
`Program.cs`, then apply the provisioned values once they are available by resolving
`OptionsUpdater<HmacAuthenticationOptions>`.

Updates are applied through the options monitor cache, so any typed client or message handler that resolves
options *after* the update observes the new values — even if a request was already made with the initial
configuration.

```csharp
// Program.cs — no credentials required yet
builder.Services.AddHmacClient<MyApiClient>();
```

```csharp
// After startup — load the provisioned settings and apply them
var provisioned = await LoadProvisionedSettingsAsync();

var updater = serviceProvider.GetRequiredService<OptionsUpdater<HmacAuthenticationOptions>>();
updater.Update<MyApiClient>(options =>
{
    options.BaseAddress = new Uri(provisioned.BaseAddress);
    options.Client = provisioned.Client;
    options.Secret = provisioned.Secret;
});

// The next request through MyApiClient uses the provisioned credentials
```

The updater also exposes `Update(Action<HmacAuthenticationOptions>)` for the default options instance,
`Update(string name, Action<HmacAuthenticationOptions>)` for a named instance, and `Reset` overloads to
revert to the originally configured values.
...
[httpYac](https://httpyac.github.io/) collection demonstrating HMAC authentication:

- **Location**: `samples/Sample.HttpYac/`
- **Features**: Automatic HMAC signature generation via config hook, public and authenticated endpoints
- **Requirements**: httpYac CLI or VS Code extension, Node.js for signature computation
- **Usage**: Open a `.http` file in VS Code and send the request, or run with the httpYac CLI
