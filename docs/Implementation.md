...
DEFAULT_SIGNED_HEADERS = ["host", "x-timestamp", "x-content-sha256", "x-nonce"]
```
...
4. **x-nonce**: A unique per-request value (GUID). Included by default in signed headers and auto-generated by the .NET client. Not server-enforced, but strongly recommended — without it, identical requests within the same second produce the same signature and are rejected when replay protection is enabled (the default).
...
4. **x-nonce** (e.g., `a3f1c2d4e5b64a7f8c9d0e1f2a3b4c5d`)
...
```text
GET\n/api/users?page=1&limit=10\napi.example.com;1640995200;47DEQpj8HBSa+/TImW+5JCeuQeRkm5NMpJWZG3hSuFU=;a3f1c2d4e5b64a7f8c9d0e1f2a3b4c5d
```

**POST Request with Body**:

```text
POST\n/api/users\napi.example.com;1640995201;jZKwqY8QqKqzQe7xJKwqY8QqKqzQe7xKwqY8QqKqzQe=;b4c5d6e7f8a9b0c1d2e3f4a5b6c7d8e9
```
...
- Example with custom headers: `host;x-timestamp;x-content-sha256;x-nonce;content-type;user-agent`
...
```text
HMAC Client=demo-client&SignedHeaders=host;x-timestamp;x-content-sha256;x-nonce&Signature=abc123def456ghi789jkl012mno345pqr678stu901vwx234yz567890=
```
...
3. Generate a unique nonce (GUID)
4. Calculate SHA256 hash of request body
5. Create header values array: [host, timestamp, content_hash, nonce]
...
10. Add all required headers to request (x-timestamp, x-content-sha256, x-nonce)
```
...
builder.Services.AddHmacAuthentication(options =>
{
    ...
    options.SignedHeaders = new[] { "host", "x-timestamp", "x-content-sha256", "x-nonce", "content-type" };
});
```
...
builder.Services.AddHmacAuthentication("UserApiAuth", options =>
{
    ...
    options.SignedHeaders = new[] { "host", "x-timestamp", "x-content-sha256", "x-nonce", "user-agent" };
});
```
...
| `SignedHeaders` | string[] | No       | Headers included in signature (defaults to host, x-timestamp, x-content-sha256, x-nonce) |

### Default Signed Headers

- **host**: Target host header (**required** — the server rejects requests that omit this from `SignedHeaders`)
...
- **x-nonce**: Unique per-request value (GUID). Not server-enforced, but included by default to ensure each signature is unique. Omitting it when replay protection is enabled (the default) causes identical requests within the same second to be rejected.
...
```csharp
// .NET
options.SignedHeaders = new[] { "host", "x-timestamp", "x-content-sha256", "x-nonce", "content-type", "user-agent" };
```
